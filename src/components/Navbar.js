...
function Navbar () {
  ...
  return (
    ...
    <nav className="bg-white shadow-md px-6 py-4 flex justify-between items-center rounded-xl">
      ...
      <div className="relative flex-1 max-w-5xl md:flex items-center bg-gray-100 rounded-lg px-4 py-3 mr-6 
  transition-all duration-150 ease-in-out focus-within:shadow-lg focus-within:translate-y-0.5">
        <img src={searchIcon} alt="Search" className="h-5 w-5 mr-2 inline-block" />
        <input
          type="text"
      ...
      <div className="flex items-center space-x-4">
        <button className="flex items-center border border-blue-500 text-black px-4 py-2 rounded-lg transition hover:bg-blue-100"     onMouseEnter={() => setIsHovered(true)}
        onMouseLeave={() => setIsHovered(false)}>
   
        <img  src={isHovered ? callHoverIcon : callIcon} alt="Call" className="h-6.5 w-6.5 mr-2" />
  ...
  <img src={logoutIcon} alt="Logout" className="h-6 w-6 cursor-pointer" />

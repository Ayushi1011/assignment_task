...
const Sidebar = () => {
  ...
  return (
    <div className={`h-screen fixed md:relative bg-white shadow-md transition-all duration-300 flex flex-col z-50 ${isOpen ? 'w-64' : 'w-20'} md:w-64 overflow-y-auto`}>
      {/* Top Section */}
      <div className="flex items-center justify-center relative p-4">
        <img src={logo} alt="Logo" className={`h-8 transition-all ${!isOpen && 'hidden md:block'}`} />
        <button onClick={() => setIsOpen(!isOpen)} className="p-2 bg-gray-200 rounded-md md:hidden">
      ...
      <nav className="mt-4 flex flex-col space-y-2">
        <SidebarLink to="/" icon={dashboardIcon} text="Dashboard" isOpen={isOpen} active />
        <SidebarLink to="#" icon={companyIcon} text="Company" isOpen={isOpen} locked />
        <SidebarLink to="#" icon={documentsIcon} text="Documents" isOpen={isOpen} locked />
        <SidebarLink to="#" icon={servicesIcon} text="Services" isOpen={isOpen} locked />
        <SidebarLink to="#" icon={complianceIcon} text="Compliance" isOpen={isOpen} locked />
        <SidebarLink to="#" icon={booksIcon} text="Banks" isOpen={isOpen} locked />
        <SidebarLink to="#" icon={moneyIcon} text="Money" isOpen={isOpen} locked />
      </nav>
  ...
  <p className="text-base font-semibold whitespace-nowrap">Your Company</p>
</div>
  ...
  return (
    <div 
      className={`flex items-center p-3 transition rounded-xl 
        ${active ? 'bg-blue-600 text-white font-bold shadow-xl' : 'text-black hover:bg-blue-100 group'}`}
      onMouseEnter={() => setIsHovered(true)}
      onMouseLeave={() => setIsHovered(false)}
    >
      <img src={icon} alt={text} className="h-6 w-6" />
      <span className={`ml-3 text-sm font-medium ${!isOpen && 'hidden md:block'}`}>{text}</span>
